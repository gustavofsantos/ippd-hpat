...
def ler_arquivos():
    ...
    for i, arquivo in enumerate(nome_arquivos):
        ...
        try:
            with open('./../pacotes-hpat/{}'.format(arquivo), encoding='utf8') as arq_json:
...
def calc_rank():
    global chaves
    global rank 
    for ic in hpat.prange(len(chaves)):
        chave = chaves[ic]
        #for chave in chaves:
        ...
        if dependencias:
            for dependencia in pacotes[chave].dependencias:
                ...
                # ---------
                # não dá para otimizar com hpat pois o dicionario rank é dependencia de dados
...
calc_rank_hpat = hpat.jit(calc_rank())
calc_rank_hpat()
...
with open('./../cache-hpat/pacotes.rank.dat', 'w') as arq_cache:
