...
from pesquisar_hpat import pesquisar, ler_cache
...
def criar_csv_medidas_tempo_mem(num_medidas_por_vez):
    ...
    for medida in range(num_medidas_por_vez):
        ...
        for pesquisa in pesquisas:
            t_a = time.time()
            resultados = pesquisar(pesquisa, 10, ler_cache())
            t_d = time.time()
            dif_t = t_d - t_a
    ...
    with open('criar_csv_medidas_tempo_hpat.csv', 'w') as saida:
        saida.write('Termo')
    ...
    with open('criar_csv_medidas_memoria_hpat.csv', 'w') as saida:
        saida.write('Termo')
        for i in range(num_medidas_por_vez):
            saida.write(',Exec {}'.format(i+1))
        saida.write('\n')
